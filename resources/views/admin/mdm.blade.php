...
<div id="panel-mdm" class="admin-panel bg-white p-4 rounded-4 border">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <div>
      ...
      <p class="text-secondary fs-09">Configure dropdown items, compliance policies, and bulk ingest records.</p>
    </div>
  </div>
  ...
  <!-- Tabs Navigation -->
  <ul class="nav nav-pills mb-4 gap-2 bg-light p-2 rounded-3 border" id="mdmTabs" role="tablist">
    ...
    <li class="nav-item" role="presentation">
      <button class="nav-link rounded-2 px-4 py-2" id="ingest-tab" data-bs-toggle="pill" data-bs-target="#ingest-pane" type="button" role="tab" aria-controls="ingest-pane" aria-selected="false">
        <i class="bi bi-cloud-arrow-up me-2"></i>Bulk Ingestion
      </button>
    </li>
  </ul>

  <!-- Tab Contents -->
  <div class="tab-content" id="mdmTabsContent">
    ...
    <!-- PANE 4: BULK INGESTION -->
    <div class="tab-pane fade animate-fade-in" id="ingest-pane" role="tabpanel" aria-labelledby="ingest-tab">
      <div class="row g-4">
        <div class="col-md-5">
...
function renderUnits() {
    fetch("{{ route('admin-mdm-get-units') }}")
        .then(res => res.json())
        .then(data => {
...
document.getElementById('add-unit-form').addEventListener('submit', function(e) {
    ...
    fetch("{{ route('admin-mdm-store-unit') }}", {
        method: 'POST',
...
document.getElementById('edit-unit-form').addEventListener('submit', function(e) {
    ...
    fetch(`/admin/mdm/units/${id}/update`, {
        method: 'POST',
        headers: requestHeaders,
        body: JSON.stringify(Object.fromEntries(formData))
    })
...
function toggleUnit(id, checkbox) {
    fetch(`/admin/mdm/units/${id}/toggle`, {
        method: 'POST',
        headers: requestHeaders
    })
    .then(res => res.json())
...
function deleteUnit(id) {
    if (confirm("Are you sure you want to delete this Business Unit? Doing so will permanently delete all its linked Sub Units!")) {
        fetch(`/admin/mdm/units/${id}/delete`, {
            method: 'POST',
            headers: requestHeaders
        })
        .then(res => res.json())
...
function renderSubUnits() {
    fetch("{{ route('admin-mdm-get-subunits') }}")
        .then(res => res.json())
        .then(data => {
...
document.getElementById('add-subunit-form').addEventListener('submit', function(e) {
    ...
    fetch("{{ route('admin-mdm-store-subunit') }}", {
        method: 'POST',
        headers: requestHeaders,
        body: JSON.stringify(Object.fromEntries(formData))
    })
...
document.getElementById('edit-subunit-form').addEventListener('submit', function(e) {
    ...
    fetch(`/admin/mdm/sub-units/${id}/update`, {
        method: 'POST',
        headers: requestHeaders,
        body: JSON.stringify(Object.fromEntries(formData))
    })
...
function toggleSubUnit(id, checkbox) {
    fetch(`/admin/mdm/sub-units/${id}/toggle`, {
        method: 'POST',
        headers: requestHeaders
    })
    .then(res => res.json())
...
function deleteSubUnit(id) {
    if (confirm("Are you sure you want to delete this Sub Unit?")) {
        fetch(`/admin/mdm/sub-units/${id}/delete`, {
            method: 'POST',
            headers: requestHeaders
        })
        .then(res => res.json())
        ...
        .catch(() => showToast("Failed to delete Sub Unit.", 'bg-danger'));
    }
}
...
// -------------------------------------------------------------
function renderGdpr() {
    fetch("{{ route('admin-mdm-get-gdpr') }}")
        .then(res => res.json())
        .then(data => {
...
document.getElementById('add-gdpr-form').addEventListener('submit', function(e) {
    ...
    fetch("{{ route('admin-mdm-store-gdpr') }}", {
        method: 'POST',
        headers: requestHeaders,
        body: JSON.stringify(Object.fromEntries(formData))
    })
...
document.getElementById('edit-gdpr-form').addEventListener('submit', function(e) {
    ...
    fetch(`/admin/mdm/gdpr/${id}/update`, {
        method: 'POST',
        headers: requestHeaders,
        body: JSON.stringify(Object.fromEntries(formData))
    })
...
function toggleGdpr(id, checkbox) {
    fetch(`/admin/mdm/gdpr/${id}/toggle`, {
        method: 'POST',
        headers: requestHeaders
    })
    .then(res => res.json())
...
function deleteGdpr(id) {
    if (confirm("Are you sure you want to delete this GDPR compliance option?")) {
        fetch(`/admin/mdm/gdpr/${id}/delete`, {
            method: 'POST',
            headers: requestHeaders
        })
        .then(res => res.json())
...
// -------------------------------------------------------------
// F. Existing Mock Upload Simulation
// -------------------------------------------------------------
